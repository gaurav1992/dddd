...
<h1 style="text-align:center;"> {!! $title or 'Rejected Driver Applicant Details' !!} </h1>
			...
			<div class="col-sm-4 pad">
				...
				<div class="col-sm-6 pad">
					...
					<div class="col-sm-5 dateofjoining suspend-button">
						...
						<p><span>Date Applied:</span>{{ $driver_data->created_at }}</p>
					</div>
  ...
  <div class="col-sm-12 pull-left seaction1-driver licence-seaction">
    <div class="col-sm-4">
      <p><span>Licence Number:</span>{{ @$driver_meta_data->license_number }}</p>
       <p><span>Licence Expiry Date:</span>{!!  date('m-d-Y', strtotime(@$user->licence_expiration)) !!}</p>
	<p><span>Insurance Expiry Date:</span>{!!  date('m-d-Y', strtotime(@$user->insurance_expiration)) !!}</p>
	 <!--Subadmin Permission Code Start-->
		<?php 
			...
			?>
				<!--Subadmin Permission Code Start-->	
				  <?php if(!empty($driver_meta_data->ssn)){@$decrypted_string =$ssn = '#####'.substr($driver_meta_data->ssn, -4); }?>
