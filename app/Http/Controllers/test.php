...
class app_test extends Controller
{
        ...
        if ($is_social == 1) {
            ...
            if ($userSocialIdCheck) {
                ...
                if($userSocialIdCheck->is_driver_approved == 1){
                    ...
                    $deleted = $firebase->delete($DEFAULT_PATH . "/drivers_location/".$userSocialIdCheck->id);
    ...
    public function pickup_request(Request $request)
    {
        ...
        $firebase = new \Firebase\FirebaseLib($const['DEFAULT_URL'], $const['DEFAULT_TOKEN']);
        // --- reading the stored string ---
        $drivers = $firebase->get($DEFAULT_PATH . '/drivers.json?orderBy="driver_available_is_hired"&equalTo="true_0"&print=pretty&print=pretty');
        ...
        if ($drivers) {
            ...
            foreach ($driver_arr as $driver_id => $driver) {
                ...
                if ($node_count >= 6 && $driver_id != $user_id && ($driver->transmission == 'both' OR $driver->transmission == "$transmission") ) {
                    ...
                    if ($driver_revoked == NULL) {
                        if ($passanger_rating > 2 AND $driver_rating > 2) {
//
    ...
    public function change_mode(Request $request)
    {
        ...
        if ($mode == 'driver') {
            ...
            $driver_data = DB::table('dn_users_data')->select('dn_users_data.transmission', 'dn_users_data.referral_code', 'dn_users_data.navigation_system', 'dn_driver_requests.licence_expiration','dn_driver_requests.insurance_expiration')
            ...
            if($userData->is_suspended == 1){
                ...
                return response()->json(['status' => 0, 'message' => 'You are suspended by admin']);
            }
            if(strtotime($driver_data->licence_expiration) < strtotime(date('Y-m-d'))){

                return response()->json(['status' => 0, 'message' => 'Your driving license has been expired!']);
            }
            if(strtotime($driver_data->insurance_expiration) < strtotime(date('Y-m-d'))){

                return response()->json(['status' => 0, 'message' => 'Your insurance has been expired!']);
            }
        }
        if ($mode == 'passenger') {
            ...
            if($userData->is_driver_approved){
                ...
                // --- reading the stored string ---
                $deleted = $firebase->delete($DEFAULT_PATH . "/drivers/$user_id");
                $deleted = $firebase->delete($DEFAULT_PATH . "/drivers_location/$user_id");
                $driver_data = (object) array();
            }
...
//                        'failOnDuplicatePaymentMethod' =>true,
            ...
            if($type == 'pickup') {
                ...
                if ($cancel_by == 'passenger') {
                    ...
                    if ($ride_time > $current_time OR in_array($subcategory, array("9", "13", "14", "16", "17"))) {
                        ...
                        $firebase->update($DEFAULT_PATH . '/drivers/' .$driver_id , ['is_hired' => '0']);
    ...
    public function pay_bill(Request $request)
    {
        ...
        if(!empty($cancellation_data)) {
            ...
            $firebase = new \Firebase\FirebaseLib($const['DEFAULT_URL'], $const['DEFAULT_TOKEN']);
            $firebase->update($DEFAULT_PATH . '/rides/' .$ride_id , ['status' => '-1', 'ride_message'=>'Ride has been cancelled']);
            $firebase->update($DEFAULT_PATH . '/drivers/' .$ride_data->driver_id , ['is_hired' => '0']);
            $check_notification = $this->sendGoogleCloudMessage('Ride has been cancelled', '8', $passenger_info, $to, $from, $ride_id);
        }
    ...
    {
        $apiKey = 'AIzaSyAu6j31zoKlQbZqtEVFIjFyW1G7kSgZAwM';   //// firebase server key
    ...
    private function firebaseConstant(){
        ...
        $DEFAULT_URL = 'https://dezinow-155207.firebaseio.com/';
        $DEFAULT_TOKEN = 'AIzaSyARd_Y0otamrOMcufrX8CI4C2hbooOElXA';
  ...
  --------------------------------------------------------------------------*/
    ...
    public function xml_client() {
        ...
        $dial = $response->dial('', array( 'callerId' => $_POST['From'], 'action' => 'http://'.$url.'logs/logfile.php?fromNo='.$_POST['From'].'&toNo='.$_POST['To']) );
